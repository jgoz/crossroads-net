namespace CrossroadsIO
{
    ...
    /// </para>
    /// <para>
    /// To actually send or receive data, the socket's <see cref="Socket.ReceiveReady"/> and/or
    /// <see cref="Socket.SendReady"/> event handlers must be attached to. If attached, these will
    /// be invoked when data is ready to be received or sent.
    /// </para>
    /// </remarks>
    public class Poller : IDisposable
    {
        private readonly Dictionary<PollItem, Socket> _pollableSockets;
        ...
        /// </summary>
        /// <param name="socketsToPoll">The collection of <see cref="Socket"/>s to poll.</param>
        public Poller(IEnumerable<Socket> socketsToPoll)
            : this()
        {
        ...
        internal Poller(PollerProxy pollerProxy)
        {
            ...
            _pollableSockets = new Dictionary<PollItem, Socket>();
        ...
        /// </summary>
        /// <param name="sockets">The collection of <see cref="Socket"/>s to poll.</param>
        public void AddSockets(IEnumerable<Socket> sockets)
        {
            if (sockets == null)
            {
        ...
        /// <see cref="Socket.ReceiveReady" /> or <see cref="Socket.SendReady" /> as appropriate.
        /// </summary>
        /// <exception cref="XsSocketException">An error occurred polling for socket events.</exception>
        public void Poll()
        {
            PollBlocking();
        }
        ...
        private int Poll(int timeoutMilliseconds)
        {
            ...
            if (readyCount > 0)
            {
                ...
                {
                    Socket socket = _pollableSockets[pollItem];
